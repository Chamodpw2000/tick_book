...
const getS3ObjectUrl = ({ bucketName, region, key }) => {
  const publicBaseUrl = process.env.S3_EVENT_PUBLIC_BASE_URL || process.env.S3_PUBLIC_BASE_URL;
  if (publicBaseUrl) {
...
const parseBase64Image = (input) => {
  if (typeof input !== "string" || input.trim().length === 0) {
    throw new Error("bannerImage must be a non-empty base64 string");
  }
...
export const uploadEventBannerBase64ToS3 = async ({ eventId, bannerImageBase64 }) => {
  const bucketName = process.env.S3_EVENT_BUCKET_NAME;
  if (!bucketName) {
    throw new Error("S3_EVENT_BUCKET_NAME is not set");
  }

  const region = process.env.AWS_REGION || "us-east-1";
  const maxBytes = Number(process.env.BANNER_IMAGE_MAX_BYTES || 5 * 1024 * 1024);

  const { contentType: declaredContentType, base64Payload } = parseBase64Image(bannerImageBase64);

  let buffer;
  try {
    buffer = Buffer.from(base64Payload, "base64");
  } catch {
    throw new Error("Invalid base64 encoding for bannerImage");
  }

  if (!buffer || buffer.length === 0) {
    throw new Error("bannerImage decoded to empty content");
  }

  if (buffer.length > maxBytes) {
    throw new Error(`bannerImage exceeds max size (${maxBytes} bytes)`);
  }
  ...
  const key = `events/${eventId}/banner-${randomUUID()}.${ext}`;
...
export const deleteEventBannerFromS3 = async ({ key }) => {
  const bucketName = process.env.S3_EVENT_BUCKET_NAME;
  if (!bucketName) {
    throw new Error("S3_EVENT_BUCKET_NAME is not set");
  }
